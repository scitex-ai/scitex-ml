...
def stx_conf_mat(
    ...
    if cm is None:
        with scitex_context.suppress_output():
    ...
    labels_to_latex = lambda labels: (
        [scitex_str.to_latex_style(label) for label in labels]
    ...
    # Use scitex_plt.subplots() for CSV export functionality
    ...
    ax = _umbrella_plt.ax.extend(ax, x_extend_ratio, y_extend_ratio)
    ...
    if spath is not None:
        ...
        scitex_io.save(fig, str(spath_abs), use_caller_path=False)
...
# Metric calculation functions have been moved to scitex_ai.metrics
# They are imported above for use in this module
# This maintains SoC: plotting in scitex_ai.plt, metrics in scitex_ai.metrics
...
def main(args):
    ...
    fig.axes[-1] = _umbrella_plt.ax.sci_note(
        fig.axes[-1],
        fformat="%3.1f",
        y=True,
    )

    scitex_io.save(fig, "plot_conf_mat_demo.jpg")
    return 0
...
if __name__ == "__main__":
    ...
    #     # To LaTeX styles
    #     if pred_labels is not None:
    #         pred_labels = [scitex_str.to_latex_style(l) for l in pred_labels]
    #     if true_labels is not None:
    #         true_labels = [scitex_str.to_latex_style(l) for l in true_labels]
    #     if labels is not None:
    #         labels = [scitex_str.to_latex_style(l) for l in labels]
    #     if sorted_labels is not None:
    #         sorted_labels = [scitex_str.to_latex_style(l) for l in sorted_labels]
...
#         cax = _umbrella_plt.ax.shift(cax, dx=-dx * 2.54, dy=0)
#         fig.add_axes(cax)
