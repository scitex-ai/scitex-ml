...
Also wires subprocess coverage at module-import time (see scitex-dev
skill leaf 05_development_06_subprocess-coverage.md). `pytest-cov` has
already set `COVERAGE_FILE` to a per-test tmp dir by the time conftest
is loaded, so `os.environ.setdefault` would be a silent no-op — we
force-set both `COVERAGE_PROCESS_START` and `COVERAGE_FILE`, then drop
an idempotent `.pth` shim into site-packages so every child Python
interpreter (`subprocess.run`, `jupyter nbconvert --execute`, etc.)
auto-starts coverage via `coverage.process_startup()`.
...
def _ensure_subprocess_coverage_shim() -> None:
    ...
    pth = purelib / "_scitex_ml_subprocess_coverage.pth"
    shim = (
        "import os, coverage\n"
